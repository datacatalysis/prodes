...
1. Residues are grouped by chain, residue sequence number **and insertion code**, which is the same key `build_structure` groups residues on. It was already the key here in version 7.0, when the parser still merged residues that differed only by an insertion code: leaving it out would have elected a single conformation across two different residues, which would have been a new defect rather than the existing one. The parser caught up in version 8.0; see `docs/residue_identity.md`.
...
**Insertion codes were ignored until version 8.0.** Residues 30 and 30A were read as one residue, which on 4NZU merged H100 through H100H into a single residue of 111 atoms holding 35 distinct atom names. The election was never affected: `residue_key` has carried the insertion code since 7.0, so a conformation was never elected across two residues sharing a number. See `docs/residue_identity.md`.
