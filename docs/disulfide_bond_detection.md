...
* **A cysteine with alternate locations.** Fixed in 7.0, and the entry is kept because what it said was only half right. The parser did read the `altLoc` column as part of the atom name, so a disordered `SG` arrived called `SG A` and was invisible to the geometric search here and to the charge calculation. It was never invisible to the `SSBOND` route, which resolves a record through its chain and residue number and reads no atom name at all, so a deposited structure carrying records found its bonds throughout. From 7.0 the parser keeps one conformation per residue, and a disordered cysteine is an ordinary `SG` on both routes. See [alternate conformations](alternate_conformations.md).
